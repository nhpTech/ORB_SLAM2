...
public:
    ...
    {
        pub_vis_pos = n.advertise<geometry_msgs::PoseStamped>(vis_pos_topic, 100);            
        pub_odom = n.advertise<nav_msgs::Odometry>(odom_topic, 100);
        pub_keyframe = n.advertise<nav_msgs::Odometry>(keyframe_topic, 100);
    ...
    // void publish(cv::Mat &im, cv::Mat &imD, cv::Mat &pose)
    // {
    //     imD.copyTo(imDepth32F);
    //     im.copyTo(imRGB_);
    //     pose.copyTo(Tcw);    
    ...
    //     const float fx = 554.254691191187, fy = 554.254691191187 , cx = 320.5, cy = 240.5; 
    ...
    void publish(cv::Mat &Tcw)
    {
        ...
        pose_stamped_msg.header.frame_id = base_frame;
        ...
        pub_vis_pos.publish(pose_stamped_msg);
        pub_odom.publish(odom_msg);
    }
    ...
    void publishAsync()
    {
        while (ros::ok())
        {
            ...
            if (!condVar.wait_for(lock, std::chrono::milliseconds(5000), std::bind(&ROSPublisher::isDataReady, this)))
            {
                if (!ros::ok()) break;
                else continue;
            }
            ...
            geometry_msgs::PoseStamped pose_stamped_msg;
            nav_msgs::Odometry odom_msg;
            nav_msgs::Odometry keyframe_msg;
            tf::poseTFToMsg(new_transform, pose_msg);
            ...
            keyframe_msg.header.stamp = time_now;
            keyframe_msg.header.frame_id = base_frame;
            keyframe_msg.child_frame_id = camera_frame;
            keyframe_msg.pose.pose = pose_msg;
            ...
            pub_vis_pos.publish(pose_stamped_msg);
            pub_odom.publish(odom_msg);
            pub_keyframe.publish(keyframe_msg);
            pub_traj.publish(traj_msg);
            pub_pc.publish(pc_msg);
    ...
    void getData(cv::Mat& im, cv::Mat& imD, cv::Mat& pose)
    {
        ...
        imD.copyTo(imDepth32F);
        // im.copyTo(imRGB_);
        cv::cvtColor(im, imRGB_, CV_RGB2GRAY);
        pose.copyTo(Tcw);    
        dataReady = true;
        condVar.notify_one();
    }
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("Using Gazebo's Kinect stream");
    ...
    while (ros::ok())
    {
        ...
        {                                
            ...
            bool isKeyframe = SLAM.isKeyframe();
