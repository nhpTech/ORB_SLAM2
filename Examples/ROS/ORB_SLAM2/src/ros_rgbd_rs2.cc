...
#include <rs_stream/CompressedRGBD.h>
...
public:
    ...
    void GrabCustomCompressedRGBD(rs_stream::CompressedRGBDConstPtr msgRGBD);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("/camera/compressed", 10, &ImageGrabber::GrabCustomCompressedRGBD, &igb);
#else
...
void ImageGrabber::GrabCustomCompressedRGBD(rs_stream::CompressedRGBDConstPtr msgRGBD)
{
    std::vector<uint8_t> rgb = msgRGBD->rgb; 
    std::vector<uint8_t> depth = msgRGBD->depth;
    cv::Mat im1 = cv::imdecode(rgb, cv::IMREAD_UNCHANGED); 
    result = zdDecompressor.Decompress(depth, width, height, decompressedD);
    if (result != zdepth::DepthResult::Success)
    {
